      ...
      for (let name in global.plugins) {
        ...
        if ((usedPrefix = (match[0] || '')[0])) {
          ...
          if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
            this.reply(m.chat, `limit kamu kurang, bantu subscribe channel kami https://youtu.be/Et5i689nJEA untuk mendapatkan 100limit klo sudah sub bisa chat owner untuk transfer limitnya di GC atau *${usedPrefix}buy*`, m)
          ...
          if (chat.welcome) {
            ...
            for (let user of participants) {
              ...
              text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'ようこそ Youkuso, @user!').replace('@subject', this.getName(jid)).replace('@desc', groupMetadata.desc) :
                ...
                mentionedJid: [user],
                externalAdReply: {
                  mediaUrl: 'https://youtube.com/channel/UC0rPHvwr0sJtaccF8Mm1Xng',
...
global.dfail = (type, m, conn) => {
  let msg = {
    ...
    premium: '❗𝘱𝘦𝘳𝘪𝘯𝘵𝘢𝘩 𝘪𝘯𝘪 𝘩𝘢𝘯𝘺𝘢 𝘣𝘪𝘴𝘢 𝘥𝘪𝘨𝘶𝘯𝘢𝘬𝘢𝘯 𝘰𝘭𝘦𝘩_*𝗨𝘀𝗲𝗿 𝗣𝗿𝗲𝗺𝗶𝘂𝗺.*\𝘯𝘭𝘢𝘯𝘨𝘨𝘢𝘯𝘢𝘯 𝘱𝘳𝘦𝘮𝘪𝘶𝘮 𝘮𝘶𝘳𝘢𝘩 5𝗸 𝘀𝗲𝗯𝘂𝗹𝗮𝗻 chat\nhttp://wa.me/6285828357727?.',
    group: 'This command only can used in Group.\nPerintah ini hanya dapat digunakan di Group.',
    private: 'Perintah ini hanya dapat digunakan di Chat Pribadi ya tong 👍.',
